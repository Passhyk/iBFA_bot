...
command -v curl > /dev/null 2>&1 || { echo >&2 "curl Not found, installing..."; sudo apt-get install curl -y; }
command -v unzip > /dev/null 2>&1 || { echo >&2 "unzip Not found, installing..."; sudo apt-get install unzip -y; }
command -v pip3 > /dev/null 2>&1 || { echo >&2 "python3-pip Not found, installing..."; sudo apt-get install python3-pip -y; }
...
do
	sleep 7s
	echo "Killing tor background proccesses..."
	changeip
	i=$(( i+1 ))
done
}
...
printf 'BRUTE FORCE ATTACK JUST BEGAN! \e[1;93m\n' && sudo service tor start && sleep 5s && checktor && main
fi
